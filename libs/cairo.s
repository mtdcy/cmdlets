...
is_listed libxcb     libs_deps && libs_args+=(-Dxcb=enabled)          || libs_args+=(-Dxcb=disabled)
...
libs_build() {
    ...
    if is_mingw; then
        ...
        cmdlet.pkgconf meson-private/cairo.pc -DCAIRO_WIN32_STATIC_BUILD
    else
        # fix missing stdc++
        cmdlet.pkgconf meson-private/cairo.pc -lstdc++
    fi

    cmdlet.pkgfile libcairo -- meson.install --tags devel
}
