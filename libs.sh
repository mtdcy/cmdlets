    ...
    for patch in "${libs_patches[@]}"; do
        case "$patch" in
            http://*|https://*)
                ...
                slogcmd "$PATCH -p${libs_patch_level:-1} -N < $file" || die "patch < $file failed."
                ;;
            *)
                slogcmd "$PATCH -p${libs_patch_level:-1} -N < $patch" || die "patch < $patch failed."
                ;;
        esac
    done

    # inline patch: always -p1
